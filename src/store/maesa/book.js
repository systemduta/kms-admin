...
const actions = {
  async index ({ commit }) {
    try {
      const { data } = await axios.get('api/web/get_book')
  ...
  async store (store, payload) {
    try {
      const { data } = await axios.post('api/web/create',
        payload,
        {
          headers: {
  ...
  async show ({commit}, id) {
    try {
      const { data } = await axios.get(`api/web/detail_book/${id}`)
  ...
  async destroy (store, id) {
    try {
      const { data } = await axios.delete(`api/web/delete_book/${id}`)
      return Promise.resolve(data)
    } catch (error) {
      return Promise.reject(error.response)
    }
