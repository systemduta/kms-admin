      ...
      <vx-card title="Question materi 1VHS">
        ...
        <vs-table pagination max-items="10" search :data="data" class="mb-2">
          ...
          <template slot="thead">
            ...
            <vs-th sort-key="totalMat">Total Materi</vs-th>
            <vs-th sort-key="totalQue">Total Pertanyaan</vs-th>
            <vs-th></vs-th>
          </template>
          <template slot-scope="{ data }">
            <vs-tr :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td :data="tr.totalMat">{{ tr.totalMat }}</vs-td>
...
export default {
  ...
  methods: {
    ...
    async detail() {
      const res = await this.dispatchIndex();
      console.log(res);
    },
  },
  mounted() {
