...
export default {
  ...
  computed: {
    ...mapState({
      data: (state) => state.performDimensi.rows,
    }),
  },
  methods: {
    ...mapActions({
      dispatchIndex: "performDimensi/index_per_3p",
      dispatchStore: "performDimensi/store",
      dispatchUpdate: "performDimensi/update",
      dispatchDestroy: "performDimensi/destroy",
      dispatchShow: "performDimensi/show",
  ...
  mounted() {
    const user_info = JSON.parse(localStorage.getItem("userInfo")).data[
      "isSuperAdmin"
    ];
    if (user_info === 1) {
      this.isAdmin = true;
