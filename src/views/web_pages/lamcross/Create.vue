<template>
  <div class="vx-row">
    <div class="w-full vx-col mb-base">
      <vx-card title="Input Data Lampiran">
        ...
        <div class="mb-5 vx-row">
          <div class="w-full vx-col">
            <small class="ml-2">Name SOP Crossfunction</small> <br>
            <v-select v-model="storeData.cross_id" :options="crossfunctions.filter(e => e.company_id==company_id)" v-validate="'required'" name="crossfunction" :reduce="e => e.id" label="title"></v-select>
...
export default {
  ...
  computed:{
    ...mapState({
      uploadProgress: state => state.lamcross.upload_progress
    })
  },
  methods:{
    ...mapActions({
      dispatchStore: 'lamcross/store',
      dispatchUpdate: 'lamcross/update',
      dispatchShow: 'lamcross/show',
      dispatchGetOrganizations: 'master/organizations',
      dispatchGetGolongans: 'master/golongans',
      dispatchGetSops: 'master/crossfunctions'
    }),
    async getMaster () {
      ...
      const cross = await this.dispatchGetSops()
      this.crossfunctions = cross.data
    },
    convertToFormData () {
    ...
    store () {
      this.$validator.validateAll().then(async res => {
        ...
        try {
          ...
          this.$router.push({name: 'lamcross'})
        } catch (error) {
          this.$vs.loading.close()
          this.isLoading = false
          this.$vs.notify({
    ...
    async getDetail () {
      ...
      this.storeData.cross_id = success.cross_id
